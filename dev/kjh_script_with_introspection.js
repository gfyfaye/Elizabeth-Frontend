...
function change_avatar(newID) {
    ...
    initialize_avatar(new_div.getAttribute('id'), new_div.getAttribute('name'), new_div.getAttribute('voice'))
}
...
async function vh_audioStarted(portal) {
    ...
    await WriteToIntrospectionFile("", avatarName);
}
...
function initialize_avatar(id, name, voice) {
    ...
    console.log(avatarName +" is on the " + side)
    ...
    setInterval(function() {
        setTimeout(async () => {
            ...
            if (json_file.length > 0 && !speaking) {
                ...
                execute_command(curr_command, 0);
            }
        }, 5000)
    }, 2000)
...
function FetchGeneratedJSON() {
	console.log("Fetching my file:" + avatarName)
...
async function WriteStatus(status) {
	fetch("https://api.the-singularity-show.com/api/write/", {
    method: "POST",
    body: JSON.stringify({file: "status", content: {"speaker": avatarName, "status": status}}),
...
async function updateMyTurn() {
    ...
    .then(response => {
        ...
        myTurn = (obj.started == avatarName) ||
                (!!obj?.done && obj?.done != avatarName) ||

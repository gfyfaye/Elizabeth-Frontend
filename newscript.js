...
async function vh_audioStarted(portal) {
    await WriteToTradingFile("", avatarID);
...
function FetchGeneratedJSON() {
	console.log("Fetching my file:" + avatarID+"next")
...
.then(response => {
    if (!speaking) {
        ...
        if (json_file.length==0) {
            speaking=false;
            console.log(`I am ready to speak, really: ${!speaking}`);
        }
    } else {

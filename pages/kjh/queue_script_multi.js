...
function switch_avatars(newID) {
    const curr_div = document.getElementById(frontID);
...
function initialize_avatar(id, name, voice) {
    ...
    myTurn = false;
    facing ="center"
    console.warn(voice_array, typeof(voice_array))
   }
...
function execute_command(command, portal) {
    ...
    if(command?action == "swtich"){
    	switch_avatars(command?newID)
    }
...
function FetchGeneratedJSON() {
	fetch('https://api.the-singularity-show.com/api/latest/', { 
		method: 'POST',
		body: JSON.stringify({file: avatarID}),

...
async function vh_audioEnded(portal) {
    console.log(`End of Audio: Finished command ${curr_command_index}`)
    tag = curr_command?.tag
    console.log(tag)

    if(curr_command?.system){
    	await WriteToIntrospectionFile(avatarID, "system", tag)
    	speaking=false;
    ...
    if (!curr_command?.continue) {
			await WriteToIntrospectionFile(avatarID, "", tag);
			myTurn = false;  
			console.log("I will wait to speak")
			await waitToSpeak()
			}
...
.then(response => {
    if (response["content"].length!=0) {
    	action = response["content"][0]["action"]
    	if (action = "PURGE"){
    		console.warn("Purging")
    		json_file =[];
    		purge = true;
    		fetch("https://api.the-singularity-show.com/api/write/", {
    			method: "POST",
    			body: JSON.stringify({file: avatarID+"Purge", content: {action: ""}}),
    			headers: {"Content-Type": "application/json"}})
    	}}})}



async function WriteToIntrospectionFile(done_str, started_str, tag) {
    fetch("https://api.the-singularity-show.com/api/write/", {
    method: "POST",
    body: JSON.stringify({
        file: "introspection",

...
function FetchJSONTextFromFile() {
    curr_command = 0;
    fetch(`../files/test3.json`) //changed to test expression -Lilah
    .then(response => response.json())
    .then(response => {json_file.push(...response["content"]); console.log(json_file);})
}
...
async function updateMyTurn() {
    ...
    .then(response => {
        ...
        // console.log("My turn to start speaking: ", response?.content?.started == avatarID);
        // console.log("Other avatars done speaking: ", (!!response?.content?.done) && (response?.content?.done != avatarID));
        })
}
